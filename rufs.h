...
#pragma once  // Preferable in C++ instead of #ifndef/#define guards

#ifdef __cplusplus
extern "C" {  // To ensure C linkage for C functions when used in C++ code
#endif
...
inline void set_bitmap(bitmap_t b, int i) {
    b[i / 8] |= 1 << (i & 7);
}

inline void unset_bitmap(bitmap_t b, int i) {
    b[i / 8] &= ~(1 << (i & 7));
}

inline uint8_t get_bitmap(bitmap_t b, int i) {
