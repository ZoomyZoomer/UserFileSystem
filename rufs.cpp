...
int dir_find(uint16_t ino, const char *fname, size_t name_len, struct dirent *dirent) {
    ...
    // If there exists data blocks already..
    bio_read(0, &sb);
    ...
    printf("ERROR: dir_find() could not find the specified directory!\n");
    return -1;
}
...
int dir_add(struct inode dir_inode, uint16_t f_ino, const char *fname, size_t name_len) {
    ...
    // If there exists at least 1 data block
    for (int i = 0; i < numBlocks; i++) {
        ...
        for (int j = 0; j < numDirs; j++) {
            ...
            // Find free data block space to put a dirent in for later
    ...
    if (dirFound == 0) {
        ...
        // Fill the data block with invalid dirents
    ...
    // Now let's add a dirent to the data block
    ...
    // Write the data block back to the disk
    bio_write(sb.d_start_blk + blockNum, block);
...
int dir_remove(struct inode dir_inode, const char *fname, size_t name_len) {
    ...
    // Step 2: Check if fname exists

    // Step 3: If exists, then remove it from dir_inode's data block and write to disk
...
int get_node_by_path(const char *path, uint16_t ino, struct inode *inode) {
    ...
    // Note: You could either implement it in an iterative way or recursive way
    ...
    while (section != nullptr) {
        ...
        // Next section
        section = strtok_r(nullptr, "/", &saveptr);
...
static void *rufs_init(struct fuse_conn_info *conn) {
    ...
    if (dev_open(diskfile_path.c_str()) == -1){
        rufs_mkfs();
    }
...
static int rufs_write(const char *path, const char *buffer, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    while (remainingSize > 0) {
        ...
        // Check if this data block is already allocated
...
int main(int argc, char *argv[]) {
    ...
    getcwd(diskfile_path.data(), PATH_MAX);
    diskfile_path.append("/DISKFILE");

    fuse_stat = fuse_main(argc, argv, &rufs_ope, nullptr);
